from mistletoe import Arg, C, Fn, Idx, If, MT, Rand, Throw, Time, V, Val, While
...
class Scenario:
    ...
    def get_height(self) -> int:
        return MT(V("height"))[0]


    def next_message(self) -> str | None:
        m = MT(V("nextMessage").Call())
        return m[0] if len(m) > 0 else None


    def set_error_probability(self, error_prob: float) -> None:
